...
def links_from_country(driver, link):
    driver.get('https://www.atlasobscura.com' + link)
    content = driver.page_source
    soup = BeautifulSoup(content, features="html.parser")

    results = []
    for card in soup.find_all("div", {"class": "card__body"}):
...
def info_from_country(driver, country_link):
    full_link = 'https://www.atlasobscura.com' + country_link
    ...
    for card in soup.find_all("div", {"class": "card__body"}):
        location = card.find('h3').text.strip()  # retrieve attraction_location
        attraction = card.find('h4').text.strip()  # retrieve attraction_name
        blurb = card.find(
            "div", {"class": "card__body-small-content"}).text.strip()  # retrieve attraction_blurb
